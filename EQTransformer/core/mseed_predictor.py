...
def mseed_predictor(
    ...
    if os.path.isdir(out_dir):
        ...
        inp = input(
            " --> Type (Yes or y) to create a new empty directory! This will erase your previous results so make a copy if you want them."
        )
    ...
    print(
        f"######### There are files for {len(station_list)} stations in {args['input_dir']} directory. #########",
        flush=True,
    )
    for ct, st in enumerate(station_list):
        ...
        predict_writer = csv.writer(
            csvPr_gen, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        ...
        csvPr_gen.flush()
        print(
            f"========= Started working on {st}, {ct+1} out of {len(station_list)} ...",
            flush=True,
        )
        ...
        for _, month in enumerate(uni_list):
            ...
            meta, time_slots, comp_types, data_set = _mseed2nparry(
                args, matching, time_slots, comp_types, st
            )
            ...
            pred_generator = PreLoadGeneratorTest(
                meta["trace_start_time"], data_set, **params_pred
            )
            ...
            for ix in range(len(predD)):
                matches, pick_errors, yh3 = _picker(
                    args, predD[ix][:, 0], predP[ix][:, 0], predS[ix][:, 0]
                )
                if (len(matches) >= 1) and (
                    (matches[list(matches)[0]][3] or matches[list(matches)[0]][6])
                ):
                    snr = [
                        _get_snr(
                            data_set[meta["trace_start_time"][ix]],
                            matches[list(matches)[0]][3],
                            window=100,
                        ),
                        _get_snr(
                            data_set[meta["trace_start_time"][ix]],
                            matches[list(matches)[0]][6],
                            window=100,
                        ),
                    ]
                    pre_write = len(detection_memory)
                    detection_memory = _output_writter_prediction(
                        meta,
                        predict_writer,
                        csvPr_gen,
                        matches,
        ...
        print(f"\n", flush=True)
        print(
            " *** Finished the prediction in: {} hours and {} minutes and {} seconds.".format(
        ...
        with open(os.path.join(save_dir, "X_report.txt"), "a") as the_file:
            ...
            the_file.write(
                "finished the prediction in:  {} hours and {} minutes and {} seconds \n".format(
                    hour, minute, round(seconds, 2)
                )
            )
...
def _mseed2nparry(args, matching, time_slots, comp_types, st_name):
    ...
    meta = {"start_time": start_time, "end_time": end_time, "trace_name": m}
...
def _output_writter_prediction(
    meta, predict_writer, csvPr, matches, snr, detection_memory, idx
):
    ...
    for match, match_value in matches.items():
        ...
        doublet = [
            st for st in detection_memory if abs((st - ev_strt).total_seconds()) < 2
        ]
        ...
        try:
            if int(pat) >= window and (int(pat) + window) < len(data):
                ...
                snr = round(
                    10
                    * math.log10(
                        (np.percentile(sw1, 95) / np.percentile(nw1, 95)) ** 2
                    ),
                    1,
                )
...
def _picker(args: dict, yh1: np.ndarray, yh2: np.ndarray, yh3: np.ndarray):
    ...
    detection = trigger_onset(
        yh1, args["detection_threshold"], args["detection_threshold"]
    )
    ...
    for ev in EVENTS:
        ...
        if int(ed - bg) >= 10:
            ...
            if len(candidate_Ss) > 1:
                candidate_Ss = {
                    list(candidate_Ss.keys())[0]: candidate_Ss[
                        list(candidate_Ss.keys())[0]
                    ]
                }
    ...
    if args["plot_mode"] == "time_frequency":
        ...
        if len(spt) > 0 and np.count_nonzero(data[:, 0]) > 10:
            ...
            for ipt, pt in enumerate(spt):
                if pt and ipt == 0:
                    pl = plt.vlines(
                        int(pt), ymin, ymax, color="c", linewidth=2, label="Picked P"
                    )
                elif pt and ipt > 0:
        ...
        if len(sst) > 0 and np.count_nonzero(data[:, 0]) > 10:
            for ist, st in enumerate(sst):
                if st and ist == 0:
                    sl = plt.vlines(
                        int(st), ymin, ymax, color="m", linewidth=2, label="Picked S"
                    )
                elif st and ist > 0:
        ...
        if pl or sl:
            ...
            plt.legend(
                custom_lines, ["E", "Picked P", "Picked S"], fancybox=True, shadow=True
            )
            plt.axis("off")
        ...
        Pxx = np.abs(Pxx)
        plt.pcolormesh(
            t, f, Pxx, alpha=None, cmap="hot", shading="flat", antialiased=True
        )
        plt.ylim(0, 40)
        ...
        if pl or sl:
            ...
            plt.legend(
                custom_lines, ["N", "Picked P", "Picked S"], fancybox=True, shadow=True
            )
            plt.axis("off")
        ...
        if pl or sl:
            ...
            plt.legend(
                custom_lines, ["Z", "Picked P", "Picked S"], fancybox=True, shadow=True
            )
            plt.axis("off")
        ...
        custom_lines = [
            Line2D([0], [0], linestyle="--", color="mediumblue", lw=2),
        ...
        spec5 = fig.add_gridspec(
            ncols=1, nrows=4, width_ratios=widths, height_ratios=heights
        )
        ...
        plt.legend(
            loc="lower center",
            bbox_to_anchor=(0.0, 1.17, 1.0, 0.102),
            ncol=3,
            mode="expand",
